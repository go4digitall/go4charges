...
export function CableTypeSelector({ selectedType, onSelect, className }: CableTypeSelectorProps) {
  return (
    <div className={cn("space-y-2", className)}>
      <label className="text-sm font-semibold text-foreground">Select your cable type:</label>
        ...
        {(Object.keys(CABLE_TYPE_INFO) as CableType[]).map((type) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs text-muted-foreground mt-1 leading-tight">{info.compatibility}</span>
            </button>
          );
        })}
      </div>
