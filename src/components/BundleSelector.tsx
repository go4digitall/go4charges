...
export function BundleSelector({ options, selectedId, onSelect, isLoading }: BundleSelectorProps) {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start gap-3 w-full">
                <RadioGroupItem value={option.id} id={option.id} className="mt-1 border-primary text-primary" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div className="text-right">
                      <span className="text-lg font-bold text-foreground">${option.price.toFixed(2)}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-sm text-muted-foreground">{option.subtitle}</span>
                    <span className="text-sm text-muted-foreground line-through">was ${option.comparePrice.toFixed(2)}</span>
                  </div>
                  {isFamily && isSelected && (
